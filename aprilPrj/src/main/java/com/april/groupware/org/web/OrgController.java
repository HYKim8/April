...
public class OrgController {
	...
	@RequestMapping(value = "admin/do_retrieve.do",method = RequestMethod.GET)
	   public String doRetrieve(SearchVO  search,Model model) {
	      ...
	      /*
	       * CodeVO code=new CodeVO(); code.setCodeTypeId("BOARD_SEARCH"); List<CodeVO>
	       * searchList=(List<CodeVO>) this.codeService.doRetrieve(code);
	       * LOG.debug("1.1=searchList="+searchList); model.addAttribute("searchList",
	       * searchList);
	       */
	         /*
	          * //페이지 사이즈: PAGE_SIZE code.setCodeTypeId("PAGE_SIZE"); List<CodeVO>
	          * pageSizeList=(List<CodeVO>) this.codeService.doRetrieve(code);
	      ...
	      return "views/admin_org_list";// "/board/board_list.jsp
	   }
	...
	@ResponseBody
	public String doUpdate(OrgVO orgVO,Locale locale) {
		LOG.debug("1=================");
		LOG.debug("1=param="+orgVO);
		LOG.debug("1=================");
				
		if( null == orgVO.getId()) {		
			throw new IllegalArgumentException("orgVO_ID를 확인 하세요.");			
		}
	 ...
	 * 
	 */
	@RequestMapping(value="admin/do_select_one.do", method=RequestMethod.POST, produces="application/json; charset=UTF-8")
	@ResponseBody
	public String doSelectOne(OrgVO orgVO) {
		LOG.debug("====================");
		LOG.debug("=doSelectOne user= : "+orgVO);
		LOG.debug("====================");
	...
	@ResponseBody
	public String doInsert(OrgVO  orgVO,Locale locale) {
		LOG.debug("1=================");
		LOG.debug("1=param="+orgVO);
		LOG.debug("1=================");
